...
if __name__ == "__main__":
    tb = TensorBoardX(config_filename_list=['pretrain_config.py'],
                      sub_dir=config.train['sub_dir'] + '/' + config.stem['model_name'])
    ...
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=30, shuffle=True, drop_last=True,
    ...
    if config.train['optimizer'] == 'Adam':
        optimizer = torch.optim.Adam(stem.parameters(), config.train['learning_rate'],
                                     weight_decay=config.loss['weight_l2_reg'])
    if config.train['optimizer'] == 'SGD':
        optimizer = torch.optim.SGD(stem.parameters(), config.train['learning_rate'],
                                    weight_decay=config.loss['weight_l2_reg'], momentum=config.train['momentum'],
                                    nesterov=config.train['nesterov'])

    if config.train['resume_optimizer'] is not None:
        last_epoch = resume_optimizer(optimizer, stem, config.train['resume_optimizer'],
                                      epoch=config.train['resume_epoch'])

    # print( optimizer.param_groups[0]['initial_lr'] )
    if config.train['use_lr_scheduler']:
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, config.train['lr_scheduler_milestones'],
                ...
                if step % config.train['log_step'] == 0:
                    ...
                    if not config.test_time:
                        ...
                        for idx, val_batch in enumerate(val_dataloader):
                            ...
                            tb.add_scalar('loss', val_loss.data.cpu().numpy(), epoch * len(train_dataloader) + step,
                                          'val')
                            tb.add_scalar('acc', val_acc.data.cpu().numpy(), epoch * len(train_dataloader) + step,
                                          'val')
                            ...
                            log_msg = "epoch {} , step {} / {} , train_loss {:.5f}, train_acc {:.2%} , val_loss {:.5f} , val_acc {:.2%} {:.1f} imgs/s".format(
                                epoch, step, len(train_dataloader) - 1, train_loss, train_acc,
                                val_loss.data.cpu().numpy()[0], val_acc.data.cpu().numpy()[0],
                                config.train['log_step'] * config.train['batch_size'] / (tt - t))
                            print(log_msg)
                            ...
                            print(
                                "epoch {} , step {} / step {} , data {:.3f}s , mv_to_cuda {:.3f}s forward {:.3f}s acc {:.3f}s loss {:.3f}s , backward {:.3f}s".format(
                                    epoch, step, len(train_dataloader), t1 - t0, t2 - t1, t3 - t2, t4 - t3, t5 - t4,
                                    t6 - t5))
                        t = tt
