...
if __name__ == "__main__":
    ...
    # input
    dataloader = torch.utils.data.DataLoader(TrainDataset(img_list), batch_size=config.train['batch_size'],
                                             shuffle=True, num_workers=8, pin_memory=True)

    G = torch.nn.DataParallel(Generator(zdim=config.G['zdim'], use_batchnorm=config.G['use_batchnorm'],
    ...
    for epoch in range(last_epoch + 1, config.train['num_epochs']):
        for step, batch in enumerate(dataloader):
            ...
            for k in batch:
                ...
                pixelwise_loss = config.loss['weight_128'] * pixelwise_128_loss + config.loss[
                    'weight_64'] * pixelwise_64_loss + config.loss['weight_32'] * pixelwise_32_loss
                ...
                tv_loss = torch.mean(torch.abs(img128_fake[:, :, :-1, :] - img128_fake[:, :, 1:, :])) + torch.mean(
                    torch.abs(img128_fake[:, :, :, :-1] - img128_fake[:, :, :, 1:]))

                cross_entropy_loss = cross_entropy(G_encoder_outputs, batch['label'])
                L_syn = config.loss['weight_pixelwise'] * pixelwise_loss + config.loss[
                    'weight_pixelwise_local'] * pixelwise_local_loss + config.loss['weight_symmetry'] * symmetry_loss + \
                        config.loss['weight_adv_G'] * adv_G_loss + config.loss['weight_identity_preserving'] * ip_loss + \
                ...
                tb.add_scalar("pixelwise_loss", pixelwise_loss.data.cpu().numpy(), epoch * len(dataloader) + step,
                              'train')
                tb.add_scalar("pixelwise_local_loss", pixelwise_local_loss.data.cpu().numpy(),
                              epoch * len(dataloader) + step, 'train')
                tb.add_scalar("symmetry_loss", symmetry_loss.data.cpu().numpy(), epoch * len(dataloader) + step,
                              'train')
                tb.add_scalar("adv_G_loss", adv_G_loss.data.cpu().numpy(), epoch * len(dataloader) + step, 'train')
                tb.add_scalar("identity_preserving_loss", ip_loss.data.cpu().numpy(), epoch * len(dataloader) + step,
                              'train')
                tb.add_scalar("total_variation_loss", tv_loss.data.cpu().numpy(), epoch * len(dataloader) + step,
                              'train')
                tb.add_scalar("cross_entropy_loss", cross_entropy_loss.data.cpu().numpy(),
                              epoch * len(dataloader) + step, 'train')
                if test_time:
                    t_numpy = time.time()
                ...
                if step % config.train['log_step'] == 0:
                    ...
                    tb.add_image_grid("grid/predict", 4, img128_fake.data.float() / 2.0 + 0.5,
                                      epoch * len(dataloader) + step, 'train')
                    tb.add_image_grid("grid/frontal", 4, batch['img_frontal'].data.float() / 2.0 + 0.5,
                                      epoch * len(dataloader) + step, 'train')
                    tb.add_image_grid("grid/profile", 4, batch['img'].data.float() / 2.0 + 0.5,
                                      epoch * len(dataloader) + step, 'train')
                    tb.add_image_grid("grid/local", 4, local_predict.data.float() / 2.0 + 0.5,
                                      epoch * len(dataloader) + step, 'train')
                    tb.add_image_grid("grid/local_input", 4, local_input.data.float() / 2.0 + 0.5,
                                      epoch * len(dataloader) + step, 'train')
