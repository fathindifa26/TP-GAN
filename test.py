...
def parse_args():
    ...
    flag_parser = parser.add_mutually_exclusive_group(
        required=False)  # whether the input images are in the format of label1/img1 label2/img2
...
if __name__ == "__main__":
    ...
    dataloader = torch.utils.data.DataLoader(TestDataset(img_list, lm_list), batch_size=args.batch_size, shuffle=False,
    ...
    for step, batch in enumerate(dataloader):
        ...
        for k in batch:
            ...
            z = Variable(
                torch.FloatTensor(np.random.uniform(-1, 1, (len(batch['img']), train_config.G['zdim']))).cuda())
            if test_time:
            ...
            img128_fake, img64_fake, img32_fake, G_encoder_outputs, local_predict, le_fake, re_fake, nose_fake, mouth_fake, local_input = G(
                ...
                batch['mouth'], z, use_dropout=False)
            if test_time:
            ...
            for i in range(img128_fake.shape[0]):
                ...
                save_image(torch.stack([img128_fake[i].data.cpu(), batch['img'][i].data.cpu(), local_predict.data.cpu(),
                                        local_input.data.cpu()),
                           '/'.join([args.resume_model, 'test', args.subdir, 'grid', img_name]))
